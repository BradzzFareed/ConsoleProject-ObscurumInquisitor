...
namespace ConsoleProject
{
    ...
    public class Game
    {
        ...
        private readonly Dictionary<string, Character> enemies; // to store enemy values using Character parameters
        private const string SAVE_FILE_PATH = "save_game.json";
        public Game()
        {
            InitializeGame();
            enemies = InitializeEnemies(); // field constructor fo Initializing Enemies
        }
        private Dictionary<string, Character> InitializeEnemies() 
        {
            return new Dictionary<string, Character>
            {
                {"Cultists", new Character("Cultists", 100, 8, 2, 20)},
                {"Mutilated Villager", new Character("Plagued Villager", 115, 10, 4, 21)},
                {"Shade", new Character("Shade", 120, 19, 8, 22)},
                {"Ghoul", new Character("Ghoul", 120, 15, 10, 25)},
                {"Shadow Fiend", new Character("Shadow Fiend", 80, 22, 7, 60)},
                {"Dreadlord", new Character("Dreadlord", 250, 35, 12, 55)} // will only be used during boss fight part
            };
        }
        ...
        private void ShowGameInterface()
        {
            ...
            GameMenu.CreateBox("What would you like to do?", 2);
        }        
                ...
                try
                {
                    ...
                    switch (selectedMove)
                    {
                        case "A": // Attack
                            ...
                            break;
                        case "D": // Defend
                            int originalDefense = currentState.Player.Defense;
                            currentState.Player.Defense *= 2;
                            GameMenu.CreateBox("You braced against the incoming attack", 5);
                            currentState.Player.Defense = originalDefense; // resets defense after enemy's attack
                            break;
                        case "S": // Special Skill (Higher variant of attack)
                    ...
                    // enemy turn
                    if (enemy.Health < 0.3 * enemy.MaxHealth)
                    {
        ...
        private void DisplayCombatStatus(Character enemy)
        {
            Console.Clear();
            ...
            while (true)
            {
                try
                {
                    ...
                    else if (!currentState.Flags.ContainsKey("chapter_1_complete"))
                    {
                        ...
                        Console.Clear();
                        Console.WriteLine("Chapter 1 Complete! Your choices will affect future events...");
            ...
            while (true)
            {
                try
                {
                    if (!currentState.Flags.ContainsKey("chapter_2_started"))
                    {
                        ...
                        GameMenu.TypingAnimation("Chapter 2: The Village");
                    ...
                    else if (!currentState.Flags.ContainsKey("chapter_2_complete"))
                    {
                        ...
                        Console.Clear();
                        GameMenu.TypingAnimation("*After meeting arthur, both of you procede to make your way towards the church*\r\n");
            ...
            while (true)
            {
                try
                {
                    if (!currentState.Flags.ContainsKey("chapter_3_started"))
                    {
                        ...
                        GameMenu.TypingAnimation("Chapter 3: The Church");
                    ...
                    {
                        currentState.Flags["chapter_3_complete"] = true;
                        currentState.CompletedEvents.Add("Chapter_3_Completed");
                        currentState.CurrentChapter++;
                        ...
                        Console.Clear();
                        Console.WriteLine("Chapter 3 Complete! Your choices will affect future events...");
        ...
        private void ChapterFour()
        {
            while (true)
            {
                try
                {
                    if (!currentState.Flags.ContainsKey("chapter_4_started"))
                    {
                        ...
                        GameMenu.TypingAnimation("\r\nDetective Zero: Have you seen a shadow?\r\n");
        ...
        private void ChapterFive()
        {
            while (true)
            {
                try
                {
                    if (!currentState.Flags.ContainsKey("chapter_5_started"))
                    {
                        currentState.Flags["chapter_5_started"] = true;

                        GameMenu.TypingAnimation("Chapter 5: Shadow? ");
                    ...
                    else if (!currentState.Flags.ContainsKey("first_shadow_encounter"))
                    {
                        ...
                        GameMenu.TypingAnimation("\r\nAnonymous voice: Please help me, I wish to become human again. I will do all that I can to aid you against them.\r\n");
            ...
            while (true)
            {
                try
                {
                    if (!currentState.Flags.ContainsKey("chapter_6_started"))
                    {
                        ...
                        GameMenu.TypingAnimation("Chapter 6: The Stone");
                    ...
                    else if (!currentState.Flags.ContainsKey("argue_with_arthur"))
                    {
                        ...
                        if (enemies.TryGetValue("Shade", out Character shade))
                        {
                    ...
                    else if (!currentState.Flags.ContainsKey("chapter_6_complete"))
                    {
                        currentState.Flags["chapter_6_complete"] = true;
                        currentState.CompletedEvents.Add("Chapter6_Completed");
                        currentState.CurrentChapter++;
                        ...
                        Console.Clear();
                        Console.WriteLine("Chapter 6 Complete! Your choices will affect future events...");
        ...
        private void ChapterSeven()
        {
            while (true)
            {
                try
                {
                    if (!currentState.Flags.ContainsKey("chapter_7_started"))
                    {
                        currentState.Flags["chapter_7_started"] = true;

                        GameMenu.TypingAnimation("Chapter 7: The Barrier");
                        
                        if (currentState.Flags.ContainsKey("memory_trigger"))
                        {
                            GameMenu.TypingAnimation("\r\n*You recall some past memories about similar experiences*\r\n");
                        ...
                        while (shadowsDefeated < symbols.Length)
                        {
                            try
                            {
                                ...
                                if (input == "L" || input == "1")
                                {
                                    if (Random.Shared.Next(2) == 0) // 50% chance to encounter an enemy
                                    {
                                ...
                                else if (input == "R" || input == "2")
                                {
                                    ...
                                    GameMenu.TypingAnimation("\nIt was not a moon symbol. Bad luck I guess.");
                                }
                                else
                                {
                                    Console.WriteLine("\nInvalid Selection. Please type 'L' or 'R'.");
                                    continue; // Loop again without incrementing shadowDefeated.
                                }
                            }
                    ...
                    {
                        currentState.Flags["chapter_7_complete"] = true;
                        currentState.CompletedEvents.Add("Chapter7_Completed");
                        currentState.CurrentChapter++;
        ...
        private void ChapterEight()
        {
            while (true)
            {
                try
                {
                    if (!currentState.Flags.ContainsKey("chapter_8_started"))
                    {
                        currentState.Flags["chapter_8_started"] = true;

                        GameMenu.TypingAnimation("Chapter 8: The Chamber");
                    ...
                    else if (!currentState.Flags.ContainsKey("chapter_8_complete"))
                    {
                        currentState.Flags["chapter_8_complete"] = true;
                        currentState.CompletedEvents.Add("Chapter8_Completed");
                        currentState.CurrentChapter++;
        ...
        private void ChapterNine()
        {
            while (true)
            {
                try
                {
                    if (!currentState.Flags.ContainsKey("chapter_9_started"))
                    {
                        currentState.Flags["chapter_9_started"] = true;

                        GameMenu.TypingAnimation("Chapter 9: The Big Tree");
                    ...
                    else if (!currentState.Flags.ContainsKey("chapter_9_complete"))
                    {
                        currentState.Flags.ContainsKey("chapter_9_complete");
                        currentState.CompletedEvents.Add("Chapter9_Completed");
                        currentState.CurrentChapter++;
                        ...
                        Console.Clear();
                        Console.WriteLine("Chapter 9 Complete! Your choices will affect future events...");
        ...
        private void ChapterTen()
        {
            while (true)
            {
                try
                {
                    if (!currentState.Flags.ContainsKey("chapter_10_started"))
                    {
                        currentState.Flags["chapter_10_started"] = true;

                        GameMenu.TypingAnimation("Chapter 10: God of Shadows");
                    ...
                    else if (!currentState.Flags.ContainsKey("finishing_blow"))
                    {
                        ...
                        GameMenu.TypingAnimation("\r\n*You proved to be victorious*");

                        GameMenu.TypingAnimation("\r\nErebus: NOOOOO! This cannot be!\r\n");
                    ...
                    else if (!currentState.Flags.ContainsKey("chapter_10_complete"))
                    {
                        currentState.Flags["chapter_10_complete"] = true;
                        currentState.CompletedEvents.Add("Chapter10_Completed");
                        currentState.CurrentChapter++;
